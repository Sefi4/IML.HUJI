...
class AdaBoost(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        self.D_ = np.full(y.size, 1 / y.size)
        self.weights_, self.models_ = [], []
        for _ in range(self.iterations_):
            f = self.wl_()
            f.fit(X, self.D_ * y)
            f_pred = f.predict(X)
            epsilon = np.sum(self.D_[f_pred != y])
            self.weights_.append(np.log((1 / epsilon) - 1) / 2)
            self.D_ *= np.exp(-y * self.weights_[-1] * f_pred)
            self.D_ /= np.sum(self.D_)
            self.models_.append(f)
